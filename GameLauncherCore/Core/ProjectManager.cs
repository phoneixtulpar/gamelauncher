	...
	public class ProjectManager : PatchCreator.IListener
	{
		...
		private class VersionComparer : IComparer<string>
		{
			public int Compare( string x, string y )
			{
				...
				if( !v1.IsValid )
					throw new FormatException( Localization.Get( LocalizationID.E_VersionCodeXIsInvalid, x ) );
				if( !v2.IsValid )
					throw new FormatException( Localization.Get( LocalizationID.E_VersionCodeXIsInvalid, y ) );
		...
		public ProjectManager( string projectRoot )
		{
			...
			if( string.IsNullOrEmpty( projectRoot ) )
				throw new ArgumentException( Localization.Get( LocalizationID.E_XCanNotBeEmpty, "'projectRoot'" ) );

			if( !PatchUtils.CheckWriteAccessToFolder( projectRoot ) )
				throw new UnauthorizedAccessException( Localization.Get( LocalizationID.E_AccessToXIsForbiddenRunInAdminMode, projectRoot ) );
			...
			utilitiesPath = this.projectRoot + PatchParameters.PROJECT_LINKS_DIRECTORY + Path.DirectorySeparatorChar;
		...
		public void CreateProject()
		{
			if( Directory.Exists( projectRoot ) )
			{
				if( Directory.GetFileSystemEntries( projectRoot ).Length > 0 )
					throw new IOException( Localization.Get( LocalizationID.E_DirectoryXIsNotEmpty, projectRoot ) );
			}
		...
		public void UpdateDownloadLinks()
		{
			if( !File.Exists( downloadLinksPath ) )
				throw new FileNotFoundException( Localization.Get( LocalizationID.E_XDoesNotExist, downloadLinksPath ) );
		...
		private PatchResult GeneratePatchInternal()
		{
			...
			{
				Log( Localization.Get( LocalizationID.E_DirectoryXMissing, versionsPath ) );
				return PatchResult.Failed;
			}

			if( !File.Exists( projectInfoPath ) )
			{
				Log( Localization.Get( LocalizationID.E_FileXMissing, projectInfoPath ) );
				return PatchResult.Failed;
			}
			...
			{
				Log( Localization.Get( LocalizationID.E_DirectoryXIsEmpty, versionsPath ) );
				return PatchResult.Failed;
			}
			...
			if( projectInfo == null )
			{
				Log( Localization.Get( LocalizationID.E_ProjectInfoCouldNotBeDeserializedFromX, projectInfoPath ) );
				return PatchResult.Failed;
			}

			if( projectInfo.Version != ProjectInfo.LATEST_VERSION )
			{
				Log( Localization.Get( LocalizationID.E_ProjectInfoOutdated ) );
				return PatchResult.Failed;
			}
			...
			if( projectInfo.CreateIncrementalPatch && versions.Length > 1 )
			{
				...
				{
					Log( Localization.Get( LocalizationID.CreatingIncrementalPatchX, Path.GetFileName( versions[i] ) + "->" + Path.GetFileName( latestVersion ) ) );
			...
			Log( Localization.Get( LocalizationID.AllPatchesCreatedInXSeconds, timer.ElapsedSeconds() ) );
