	...
	public class PatchUpdater
	{
		...
		public PatchUpdater( string versionInfoPath, LogEvent logger = null )
		{
			...
			if( string.IsNullOrEmpty( versionInfoPath ) )
				throw new ArgumentException( Localization.Get( LocalizationID.E_XCanNotBeEmpty, "'versionInfoPath'" ) );

			if( !File.Exists( versionInfoPath ) )
				throw new FileNotFoundException( Localization.Get( LocalizationID.E_PatchInfoDoesNotExistAtX, versionInfoPath ) );

			if( !PatchUtils.CheckWriteAccessToFolder( Path.GetDirectoryName( versionInfoPath ) ) )
				throw new UnauthorizedAccessException( Localization.Get( LocalizationID.E_AccessToXIsForbiddenRunInAdminMode, versionInfoPath ) );
			...
			if( VersionInfo == null )
				throw new InvalidOperationException( Localization.Get( LocalizationID.E_VersionInfoCouldNotBeDeserializedFromX, versionInfoPath ) );
		...
		public bool UpdateDownloadLinks( string downloadLinksPath )
		{
			...
			catch( Exception e )
			{
				...
				{
					Logger( e.ToString() );
					Logger( Localization.Get( LocalizationID.E_CouldNotReadDownloadLinksFromX, downloadLinksPath ) );
				}
			...
			for( int i = 0; i < downloadLinksSplit.Length; i++ )
			{
				...
				{
					if( Logger != null )
						Logger( Localization.Get( LocalizationID.E_DownloadLinkXIsNotValid, downloadLinkRaw ) );
		...
		public bool UpdateDownloadLinks( Dictionary<string, string> downloadLinks )
		{
			...
			if( Logger != null )
				Logger( Localization.Get( LocalizationID.XDownloadLinksAreUpdatedSuccessfully, updateCount, totalCount ) );
