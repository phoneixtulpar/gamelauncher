...
namespace GameLauncherCore
{
	...
	{
		public RepairPatchApplier( PatchManager comms ) : base( comms )
		{
		}
		...
		protected override PatchResult Execute()
		{
			...
			comms.Log( Localization.Get( LocalizationID.CalculatingFilesToDownload ) );
			...
			if( filesToDownload.Count > 0 && comms.VerifyFiles )
			{
				...
				for( int i = 0; i < filesToDownload.Count; i++ )
				{
					...
					if( !comms.DownloadManager.FileExistsAtUrl( comms.VersionInfo.GetDownloadURLFor( item ), out fileSize ) )
					{
						...
						comms.FailDetails = Localization.Get( LocalizationID.E_FileXDoesNotExistOnServer, item.Path );
					...
					else if( fileSize > 0L && fileSize != item.CompressedFileSize )
					{
						...
						comms.FailDetails = Localization.Get( LocalizationID.E_FileXIsNotValidOnServer, item.Path );
			...
			if( filesToDownload.Count > 0 )
				comms.Log( Localization.Get( LocalizationID.DownloadingXFiles, filesToDownload.Count ) );
			...
			comms.Log( Localization.Get( LocalizationID.AllFilesAreDownloadedInXSeconds, downloadTimer.ElapsedSeconds() ) );
		...
		private bool DownloadAndUpdateFiles( List<VersionItem> filesToDownload, List<VersionItem> filesToUpdate )
		{
			...
			for( int i = 0, j = 0; i < filesToUpdate.Count; i++ )
			{
				...
				if( filesToDownload.Count > 0 && filesToDownload[j] == item )
				{
					...
					comms.Log( Localization.Get( LocalizationID.DownloadingXthFile, j + 1, filesToDownload.Count, item.Path, item.CompressedFileSize.ToMegabytes() ) );
					Stopwatch downloadTimer = Stopwatch.StartNew();
					...
					if( downloadedFile == null )
					{
						...
						comms.FailDetails = Localization.Get( LocalizationID.E_XCouldNotBeDownloaded, item.Path );
					...
					else if( !downloadedFile.MatchesSignature( item.CompressedFileSize, item.CompressedMd5Hash ) )
					{
						...
						comms.FailDetails = Localization.Get( LocalizationID.E_DownloadedFileXIsCorrupt, item.Path );

						return false;
					}
					else
						comms.Log( Localization.Get( LocalizationID.XDownloadedInYSeconds, item.Path, downloadTimer.ElapsedSeconds() ) );
				...
				comms.Log( Localization.Get( LocalizationID.UpdatingXthFile, i + 1, filesToUpdate.Count, item.Path ) );
