...
namespace GameLauncherCore
{
	internal class IncrementalPatchApplier : PatchMethodBase
	{
		...
		public IncrementalPatchApplier( PatchManager comms, IncrementalPatchInfo patchInfo ) : base( comms )
		{
		...
		protected override PatchResult Execute()
		{
			...
			if( patchInfo.Files.Count > 0 )
			{
				...
				if( !patchFile.Exists || !patchFile.MatchesSignature( patchInfo.CompressedFileSize, patchInfo.CompressedMd5Hash ) )
				{
					...
					comms.Log( Localization.Get( LocalizationID.DownloadingPatchX, patchInfo.PatchVersion() ) );
					...
					if( patchFile == null )
					{
						...
						comms.FailDetails = Localization.Get( LocalizationID.E_PatchXCouldNotBeDownloaded, patchInfo.PatchVersion() );
					...
					else if( !patchFile.MatchesSignature( patchInfo.CompressedFileSize, patchInfo.CompressedMd5Hash ) )
					{
						...
						comms.FailDetails = Localization.Get( LocalizationID.E_DownloadedFileXIsCorrupt, patchInfo.PatchVersion() );

						return PatchResult.Failed;
					}
					else
						comms.Log( Localization.Get( LocalizationID.XDownloadedInYSeconds, patchInfo.PatchVersion(), downloadTimer.ElapsedSeconds() ) );
				}
				else
				...
				comms.Log( Localization.Get( LocalizationID.DecompressingPatchX, patchInfo.PatchVersion() ) );
				...
				comms.Log( Localization.Get( LocalizationID.UpdatingXFiles, patchInfo.Files.Count ) );
				...
				for( int i = 0; i < patchInfo.Files.Count; i++ )
				{
					...
					{
						comms.Log( Localization.Get( LocalizationID.E_DiffOfXDoesNotExist, Path.GetFileName( fileRelativePath ) ) );
				...
				comms.Log( Localization.Get( LocalizationID.XFilesUpdatedSuccessfully, patchInfo.Files.Count - failedItemCount, patchInfo.Files.Count ) );
			}

			if( patchInfo.RenamedFiles.Count > 0 )
			{
				comms.Log( Localization.Get( LocalizationID.RenamingXFiles, patchInfo.RenamedFiles.Count ) );
				if( !RenameItems( patchInfo.RenamedFiles ) )
					return PatchResult.Failed;
			}

			// Updating version code to the latest one will be done by GameLauncherCore, after checking
		...
		private bool ApplyDiffToFile( string filePath, string diffPath, string targetPath, int patchItemIndex )
		{
			...
			{
				comms.Log( Localization.Get( LocalizationID.AlreadyUpToDateXthFile, patchItemIndex + 1, patchInfo.Files.Count, item.Path ) );
				return true;
			}

			if( item.BeforeFileSize == 0L )
			{
				comms.Log( Localization.Get( LocalizationID.CreatingXthFile, patchItemIndex + 1, patchInfo.Files.Count, item.Path ) );
			...
			comms.Log( Localization.Get( LocalizationID.UpdatingXthFile, patchItemIndex + 1, patchInfo.Files.Count, item.Path ) );
