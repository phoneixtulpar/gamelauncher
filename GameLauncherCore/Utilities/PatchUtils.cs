...
namespace GameLauncherCore
{
	public static class PatchUtils
	{
        ...
        /// <summary>
        /// Set the folder as a hidden folder
        /// </summary>
        /// <param name="path"></param>
        public static void SetFolderHidden(string path, bool hidden = true)
        {
            if (Directory.Exists(path))
            {
                DirectoryInfo directoryInfo = new DirectoryInfo(path);

                // If "hidden" is true, set as hidden, if not, set as visible
                if (hidden)
                {
            ...
            {
                Console.WriteLine("Folder doesn't exists: " + path);
            }
        }
        #endregion
    }
