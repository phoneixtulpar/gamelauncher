...
namespace GameLauncherCore
{
	...
	{
		public readonly GameLauncherPatcher Patcher;
        ...
        public VersionInfo VersionInfo
        {
            ...
            set
            {
				...
				string CachePathRoot = PatchUtils.GetPathWithTrailingSeparatorChar(applicationDirectory) +
					"temp_GameLauncherCoreDownloads";
                CachePath = CachePathRoot + Path.DirectorySeparatorChar + value.Name + Path.DirectorySeparatorChar;

                if (!Directory.Exists(CachePathRoot))
                {
                    DirectoryInfo di = Directory.CreateDirectory(CachePathRoot);
        ...
        public GameLauncherPatcher.IListener Listener;
		...
		public PatchManager( GameLauncherPatcher patcher, string rootPath )
		{
			Patcher = patcher;
			RootPath = rootPath;
		...
		public bool IsUnderMaintenance()
		{
			...
			if( underMaintenance == MaintenanceCheckResult.Maintenance_AbortApp )
			{
				...
				FailDetails = Localization.Get( LocalizationID.E_ServersUnderMaintenance );
